...
class Utils(object):
    ...
    def generate_pytorch_file(cls, indi):
        ...
        unit_list = []
        layer_bone_1 = "self.conv = nn.Conv2D(%d, %d, kernel_size=7, stride=1, padding=3, bias_attr=False)" % \
                       (StatusUpdateTool.get_input_channel(), StatusUpdateTool.get_first_channel())
        layer_bone_2 = "self.bn1 = nn.BatchNorm2D(%d)" % (StatusUpdateTool.get_first_channel())
        unit_list.append(layer_bone_1)
        unit_list.append(layer_bone_2)
