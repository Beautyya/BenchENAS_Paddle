...
class Utils(object):
    ...
    def read_template(cls):
        ...
        line = f.readline().rstrip()  # skip the comment '#generated_init'
        while line.strip() != '# generate_summary':
            part3.append(line)
            line = f.readline().rstrip()
    ...
    def generate_pytorch_file(cls, indi):
        ...
        for index, u in enumerate(indi.units):
            if u.type == 1:
                ...
                layer = 'self.op%d = nn.Conv2D(in_channels=%d, out_channels=%d, kernel_size=(%d, %d), stride=(%d, %d), ' \
                        'padding=0, weight_attr=nn.initializer.Normal(mean=%f,std=%f))' % (
                            index, u.in_channel, u.out_channel, u.filter_size[0],
                            u.filter_size[1], u.stride_size[0],
                            u.stride_size[1], u.mean, u.std)
                unit_list.append(layer)
            elif u.type == 3:
                layer = 'self.op%d = nn.Linear(in_features=%d, out_features=%d, ' \
                        'weight_attr=nn.initializer.Normal(mean=%f,std=%f))' % (
                            index, u.input_neurons_number, u.output_neurons_number, u.mean, u.std)
                unit_list.append(layer)
        # print('\n'.join(unit_list))
        ...
        summary_ = 'paddle.summary(model, (16, %d, %d, %d))' % (StatusUpdateTool.get_input_channel(), image_output_size,
                                                                image_output_size)
        for u in indi.units:
            if u.type == 1:
        ...
        for i, u in enumerate(indi.units):
            ...
            if u.type == 1:
                ...
                if is_first_fc:
                    _str = 'out_%d = paddle.reshape(out_%d,[out_%d.shape[0], -1])' % (i - 1, i - 1, i - 1)
                    forward_list.append(_str)
                    is_first_fc = False
                _str = 'out_%d = self.op%d(%s)' % (i, i, last_out_put)
                forward_list.append(_str)
