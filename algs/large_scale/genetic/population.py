...
class ArcText():
    ...
    def to_file(self):
        ...
        for unit in self.unitlist:
            if unit.Type == 'CONV':
                _str.append(
                    'self.unit_%d = nn.Conv2D(%d, %d, kernel_size=(%d, %d), stride=%d, padding=(%d, %d))' % (
                        ...
                        unit.padding[1]))
            if unit.Type == 'FC':
                _str.append('self.unit_%d = nn.Linear(%d, %d)' % (unit.Id, unit.in_size, unit.out_size))
            if unit.Type == 'BN_RELU':
                _str.append(
                    'self.unit_%d = nn.Sequential(nn.BatchNorm2D(%d), nn.ReLU())' % (
        ...
        for unit in self.unitlist:
            ...
            if unit.Type == 'INTERPOLATE':
                _str.append('x[%d] = nn.functional.interpolate(x[%d], size=[%d, %d])' % (
                    unit.Id, unit.Id, unit.out_size[0], unit.out_size[1]))
            if unit.Type == 'BN_RELU':
                _str.append('x[%d] = self.unit_%d(x[%d])' % (unit.Id, unit.Id, unit.Id))
            if unit.Type == 'GLOBAL_POOL':
                _str.append('x[%d] = paddle.squeeze(paddle.squeeze(paddle.nn.AdaptiveAvgPool2D((1, 1))(x[%d]), 3), 2)' %
                            (unit.Id, unit.Id))
            if unit.Type == 'FC':
                _str.append('x[%d] = self.unit_%d(x[%d])' % (unit.Id, unit.Id, unit.Id))
            for to_vertex in unit.Cat_to:
                if to_vertex.visit == False:
                    ...
                    _str.append('paddle.concat(x=[x[%d], x[%d]], axis=1)' % (to_vertex.Id, unit.Id))
