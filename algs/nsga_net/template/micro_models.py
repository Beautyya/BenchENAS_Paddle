...
def drop_path(x, drop_prob):
    if drop_prob > 0.:
        ...
        ten = paddle.to_tensor(x.shape[0], 1, 1, 1).astype('float32')
        mask = paddle.to_tensor(ten.bernoulli_(keep_prob), stop_gradient=False
        x.div_(keep_prob)
        x.mul_(mask)
    return x
...
class Cell(nn.Layer):
    ...
    def forward(self, s0, s1, drop_prob):
        ...
        if self.se_layer is None:
            return paddle.concat(x=[states[i] for i in self._concat], axis=1)
        else:
            return self.se_layer(paddle.concat(x=[states[i] for i in self._concat], axis=1))
...
class AuxiliaryHeadCIFAR(nn.Layer):
    ...
    def forward(self, x):
        x = self.features(x)
        x = self.classifier(paddle.reshape(x,[x.size(0), -1]))
        return x
...
class NetworkCIFAR(nn.Layer):
    ...
    def forward(self, input):
        ...
        logits = self.classifier(paddle.reshape(out,[out.size(0), -1]))
        return logits, logits_aux


class PyramidNetworkCIFAR(nn.Layer):
