...
class LOSHourGlassDecoder(nn.Layer):
    __metaclass__ = HourGlassDecoder
    ...
    def __init__(self, genome, n_stacks, out_feature_maps, pre_hourglass_channels=32, hourglass_channels=64):
        ...
        self.initial = nn.Sequential(
            nn.Conv2D(3, self.pre_hourglass_channels, kernel_size=7, stride=2, padding=3, bias_attr=True),
        ...
        to_score_map = [nn.Conv2D(self.hourglass_channels, out_feature_maps, kernel_size=1, bias_attr=True)]
        # 1x1 convs to adjust scoremap back to appropriate feature map count.
        from_score_map = [nn.Conv2D(out_feature_maps, self.hourglass_channels + self.pre_hourglass_channels,
        ...
        for i in range(1, n_stacks):
            ...
            to_score_map.append(nn.Conv2D(self.hourglass_channels, out_feature_maps, kernel_size=1, bias_attr=True))
            ...
            if i < n_stacks - 1:
                skip_convs.append(nn.Conv2D(hg_channels, hg_channels, kernel_size=1, bias_attr=True))
        ...
        self.first_lin = nn.LayerList(first_lin)
        self.to_score_map = nn.LayerList(to_score_map)
        self.from_score_map = nn.LayerList(from_score_map)
        self.second_lin = nn.LayerList(second_lin)
        self.skip_convs = nn.LayerList(skip_convs)
    ...
    def forward(self, x):
        ...
        for i in range(self.n_stacks):
            ...
            if i < self.n_stacks - 1:
                ...
                a = paddle.concat(x=[y, skip], axis=1)
...
class Lin(nn.Layer):
    def __init__(self, in_channels, out_channels):
        ...
        self.model = nn.Sequential(
            nn.Conv2D(in_channels, out_channels, kernel_size=1, bias_attr=True),
...
class LOSHourGlassBlock(nn.Layer):
    ...
    def __init__(self, graph, in_channels, out_channels, operating_channels=64):
        ...
        self.samplers = nn.LayerList(samplers)
        ...
        self.skip_ops = nn.LayerList(skip_ops)
        ...
        self.path_ops = nn.LayerList(path_ops)
...
class HourGlassResidual(nn.Layer):
    def __init__(self, in_channels, out_channels):
        ...
        self.skip_layer = Identity() if in_channels == out_channels else \
            nn.Sequential(nn.Conv2D(in_channels, out_channels, kernel_size=1, bias_attr=True),
                          nn.BatchNorm2D(out_channels))

        self.model = nn.Sequential(
            nn.Conv2D(in_channels, out_channels // 2, kernel_size=1, bias_attr=True),
            nn.BatchNorm2D(out_channels // 2),
            nn.ReLU(),
            nn.Conv2D(out_channels // 2, out_channels // 2, kernel_size=3, stride=1, padding=1, bias_attr=True),
            nn.BatchNorm2D(out_channels // 2),
            nn.ReLU(),
            nn.Conv2D(out_channels // 2, out_channels, kernel_size=1, bias_attr=True),
            nn.BatchNorm2D(out_channels)
        )
...
class ResidualPhase(nn.Layer):
    ...
    def __init__(self, gene, in_channels, out_channels, idx, preact=False):
        ...
        self.first_conv = nn.Conv2D(in_channels, out_channels, kernel_size=1 if idx != 0 else 3, stride=1, bias_attr=False)
        ...
        for i in range(len(gene)):
            if len(self.dependency_graph[i + 1]) > 0:
                ...
                none_list.append(i)  # Module list will ignore NoneType.

        self.nodes = nn.LayerList(nodes)
        if not none_list:
            for x in none_list:
                self.nodes.insert(x, 'None')
        ...
        for node_idx, dependencies in self.dependency_graph.items():
            if len(dependencies) > 1:
                conv1x1s[node_idx] = \
                    nn.Conv2D(len(dependencies) * out_channels, out_channels, kernel_size=1, bias_attr=False)

        self.processors = nn.LayerList(conv1x1s)
    ...
    def process_dependencies(self, node_idx, outputs):
        if self.dependency_graph[node_idx]:
            res = paddle.concat(x=[outputs[i] for i in self.dependency_graph[node_idx]], axis=1)
        else:
            res = paddle.concat(x=[outputs[0]], axis=1)

        return self.processors[node_idx](res)


class ResidualNode(nn.Layer):
    def __init__(self, in_channels, out_channels, stride=1,
                 kernel_size=3, padding=1, bias=False):
        super(ResidualNode, self).__init__()

        self.model = nn.Sequential(
            nn.Conv2D(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias_attr=bias),
            nn.BatchNorm2D(out_channels),
            nn.ReLU()
        )
...
class PreactResidualNode(nn.Layer):
...
class DensePhase(nn.Layer):
    def __init__(self, gene, in_channels, out_channels, idx):
        ...
        self.last_conv = nn.Conv2D(channel_adjustment, out_channels, kernel_size=1, stride=1, bias_attr=False)
    ...
    def forward(self, x):
        ...
        for i in range(1, len(self.nodes) + 1):
            if not self.dependency_graph[i]:  # Empty dependencies, no output to give.
                ...
                outputs.append(self.nodes[i - 1](paddle.concat(x=[outputs[j] for j in self.dependency_graph[i]], axis=1)))
        ...
        if self.out_channel_flag and 0 in self.dependency_graph[len(self.nodes) + 1]:
            ...
            return self.out(paddle.concat(x=[outputs[i] for i in non_zero_dep] + [outputs[0]], axis=1))

        if self.out_channel_flag:
            # Same as above, we just don't worry about the 0th node.
            return self.out(paddle.concat(x=[outputs[i] for i in self.dependency_graph[len(self.nodes) + 1]], axis=1))

        return self.out(paddle.concat(x=[outputs[i] for i in self.dependency_graph[len(self.nodes) + 1]]))


class DenseNode(nn.Layer):
    ...
    def __init__(self, in_channels):
        ...
        self.model = nn.Sequential(
            nn.BatchNorm2D(in_channels),
            nn.ReLU(),
            nn.Conv2D(in_channels, self.t * self.k, kernel_size=1, bias_attr=False),
            nn.BatchNorm2D(self.t * self.k),
            nn.ReLU(),
            nn.Conv2D(self.t * self.k, self.t, kernel_size=3, stride=1, padding=1, bias_attr=False)
        )
...
class EvoNetwork(nn.Layer):
    def __init__(self, genome, channels, out_features, data_shape, decoder="residual", repeats=None):
        ...
        out = self.model(paddle.to_tensor(paddle.zeros(shape=[1, channels[0][0], *data_shape]), stop_gradient=False))
        shape = out.shape
