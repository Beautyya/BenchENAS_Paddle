            ...
            for i, l in enumerate(layers):
                ...
                if l_type == 'conv':
                    ...
                    for fpos in dsp_list[pos]:
                        if ic > self.max_cat and dpc_list[i] > 0:
                            # adjust the channel
                            layer_forward += '\n%sx%d = paddle.concat(x=[x%d, self.adjust%d_%d(x%d)], axis=1)' % (
                            space_str, pos - 1, pos - 1, fpos, pos, fpos)
                        else:
                            layer_forward += '\n%sx%d = paddle.concat(x=[x%d, x%d], axis=1)' % (
                    ...
                    if last_layer_type == 'conv' or last_layer_type == 'maxp' or last_layer_type == 'avgp':
                        layer_forward += '\n%sx%d = paddle.reshape(x%d, [len(x%d),-1])' % (space_str, pos - 1, pos - 1, pos - 1)

                    if i == 0:
                        layer_forward += '\n%sx%d = F.relu(self.linear%d(x))' % (space_str, pos, pos)
                    else:
                        layer_forward += '\n%sx%d = F.relu(self.linear%d(x%d))' % (
