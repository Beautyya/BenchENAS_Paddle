...
class CIFAR100(BaseDataloader):
    ...
    def __get_train_valid_loader(self,
        ...
        train_dataset = datasets.Cifar100(
            data_file=data_dir, mode="train",
            download=self.download, transform=train_transform,
        )

        valid_dataset = datasets.Cifar100(
            data_file=data_dir, mode="test",
        ...
        train_sampler = paddle.io.Subset(dataset=indices, indices=indices[split:])
        valid_sampler = paddle.io.Subset(dataset=indices, indices=indices[:split])

        train_loader = paddle.io.DataLoader(
            train_dataset, batch_size=batch_size, batch_sampler=train_sampler,
        )
        valid_loader = paddle.io.DataLoader(
            valid_dataset, batch_size=batch_size, batch_sampler=valid_sampler,
        )
